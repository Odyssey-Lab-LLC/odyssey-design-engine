...
- **Needs dev server:** No
- **Expected state before phase:** Either (not running preferred)
- **Start command:** N/A (not needed this phase)
- **After phase:** Still stopped
- **Why:** This phase only installs dependencies and creates files - no visual validation needed
...
- Backup target: `sites/odyssey-lab/src/App.jsx.backup`
- New directory: `sites/odyssey-lab/src/pages/`
- Vercel config: `vercel.json` (project root)
...
- [ ] If cost exceeds $3, STOP (something is wrong - this is simple setup)
...
```markdown
| 1 | `002-phase-1-setup.md` | ✅ Complete | [date/time] | [date/time] | $[X] | [X] mins | [issues or "None"] |
```
...
- If exceeds estimate by 50% (>22 mins), STOP and reassess
...
**Green flag:** Cost at $2, phase completes → Continue (worth it)
**Yellow flag:** Cost at 1.5× $2, phase 50% done → Continue cautiously
**Red flag:** Cost at 2× $2, phase <25% done → STOP (something wrong)

**Better abort signal: Time stuck without progress**
- >10 mins debugging same error with no file changes → STOP
...
- Next phase: `plans/handoffs/002-phase-2-migrate-home.md`
