...
phase_number: 4
phase_name: Convert andrew-gem-2 → ThresholdConvergence.jsx
from_agent: Phase 3 (Convert andrew-1)
to_agent: Execution Agent
date: 2026-01-03
estimated_time: 50-60 mins
estimated_cost: $6-8
dependencies: Phase 3 complete, CallToAdventure.jsx exists and works
---

# Phase 4: Convert andrew-gem-2.jsx → ThresholdConvergence.jsx

## 🎯 Phase Objective

**Convert the complex andrew-gem-2 file to a working React component with all interactive features (lens, matrix text, zones, Lenis) OR working fallbacks.**
...
### Step 4.9: Visual Comparison (Comprehensive)
...
2. **Open converted (temporary App.jsx mod):**
   ```jsx
   import ThresholdConvergence from './pages/ThresholdConvergence';
   function App() { return <ThresholdConvergence />; }
   ```

3. **Visual checks:**
   - [ ] Lens hero effect renders (or static fallback centered)
   - [ ] Zone transitions work (or light zone default)
   - [ ] Matrix text animation plays (or static text)
   - [ ] Typography and spacing match
   - [ ] Colors exact (especially dark tones)
   - [ ] Tailwind classes still work (after CDN removal)

4. **Interaction test:**
   - [ ] Lens mouse tracking (or static)
   - [ ] Smooth scroll (or native)
   - [ ] Zone switcher triggers (or static)
   - [ ] Accordions work
   - [ ] Hover states work
...
### Step 4.10: Revert App.jsx

**What:** Restore original App.jsx.
...
**Create note for lessons learned:**
```markdown
...
**Validation C: Core Visual Parity**
- [ ] Layout matches original
- [ ] Colors correct
- [ ] Content visible

**Validation D: Interactions (Some May Be Fallbacks)**
- [ ] Accordions work
- [ ] Basic scroll works
- [ ] Page is usable
...
- [ ] If total phase time exceeds 90 mins, STOP
...
- Any deviations: [Document issues]

**Update meta-plan status:**
```markdown
| 4 | `002-phase-4-convert-andrew-gem-2.md` | ✅ Complete | [date/time] | [date/time] | $[X] | [X] mins | [issues or "Fallbacks: X, Y, Z"] |
```
...
- Hard limit: 90 minutes (STOP if exceeded)
- Fallback time budget: 15 mins total across all features

## 💰 Cost Budget

- Estimated: $6-8
- If exceeds $12, STOP and report
...
- Next: `plans/handoffs/002-phase-5-router-integration.md`
