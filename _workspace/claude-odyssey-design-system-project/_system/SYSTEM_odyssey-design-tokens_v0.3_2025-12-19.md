...
changelog: |
  v0.3 â€” Added focus state tokens, motion tokens, expanded slot architecture (leading, badge/meta, supporting, action-cue), pattern extension decision tree, quick reference appendix
  v0.2 â€” Added hero variants, guide presence modes, candidate pattern system, unified typography from both source artifacts
---
```

# Odyssey Lab Unified Design System
## Token Documentation & Component Architecture v0.3
...
```css
/* @PRESERVE â€” Odyssey Lab Master Color Tokens v0.3 */
:root {
  ...
  /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     FOCUS & ACCESSIBILITY â€” Interactive state tokens
     â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
  
  /* @PRESERVE â€” Focus State Tokens (v0.3) */
  --focus-ring-color: var(--color-lab-blue);
...
```css
/* @PRESERVE â€” Zone System Classes v0.3 */
...
:root {
  ...
  /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     MOTION TOKENS â€” Animation timing (v0.3)
     â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
  
  /* @PRESERVE â€” Motion Tokens */
  --duration-fast: 150ms;
...
**Component Anatomy (v0.3)**:
```
[Component]
â”œâ”€â”€ Base structure (required)
â”œâ”€â”€ Slots (optional content zones â€” in visual flow order)
â”‚   â”œâ”€â”€ leading        â† Visual identifier (avatar, icon) â€” appears first
â”‚   â”œâ”€â”€ badge/meta     â† Status indicators, timestamps, category tags
â”‚   â”œâ”€â”€ eyebrow        â† Pre-header (mono, uppercase) â€” use eyebrow OR subheading, not both
â”‚   â”œâ”€â”€ heading        â† Primary title
â”‚   â”œâ”€â”€ subheading     â† Post-header â€” use subheading OR eyebrow, not both
â”‚   â”œâ”€â”€ body           â† Main content
â”‚   â”œâ”€â”€ supporting     â† Help text, captions, secondary info
â”‚   â”œâ”€â”€ media          â† Images, video, visualizations
â”‚   â”œâ”€â”€ actions        â† Buttons, links, interactive elements
â”‚   â”œâ”€â”€ action-cue     â† Motivator/reinforcer below actions ("What happens next", "Why click")
â”‚   â””â”€â”€ footer         â† Closing metadata, timestamps, attribution
...
| Slot | Usage Context |
|------|---------------|
| `leading` | Avatar before heading, icon identifier, entity thumbnail |
| `badge/meta` | "New" badges, timestamps like "2 hours ago", category pills |
| `eyebrow` | Pre-header like "PHASE 01" or "// Strategy" â€” mutually exclusive with subheading |
| `subheading` | Post-header subtitle â€” mutually exclusive with eyebrow |
| `supporting` | Form field help text, "Learn more" hints, caption for above content |
| `action-cue` | "You'll receive confirmation via email", "Risk-free trial", friction reducers |
...
.hero__portal-ring {
  ...
  border-radius: var(--radius-full);
  animation: pulse var(--duration-slow) infinite ease-in-out;
  pointer-events: none;
  z-index: 0;
}
...
| Motion | `--duration-[speed]` or `--easing-[name]` | `--duration-slower` |
...
### 8.5 Pattern Extension Decision Tree (v0.3)

When content needs deviate from existing patterns, follow this decision flow:

```
1. Can existing CSS classes/tokens handle this as-is?
   â†’ YES: Use existing patterns directly
   â†’ NO: Continue to step 2

2. Is the deviation purely STYLISTIC? (color, spacing, sizing)
   â†’ YES: Use token overrides or add variant class
   â†’ NO: Continue to step 3

3. Is the deviation in SLOT USAGE? (different content arrangement)
   â†’ YES: Use the slot architecture flexibly â€” slots are optional guides, not rigid requirements
   â†’ NO: Continue to step 4

4. Is this a STRUCTURAL pattern not covered by existing CSS?
   â†’ YES: Create new CSS with @CANDIDATE flag
   â†’ NO: Compose from existing patterns

5. Is this a ONE-TIME need or RECURRING pattern?
   â†’ ONE-TIME: Build locally, don't add to system
   â†’ RECURRING (3+ uses): Flag for potential promotion
```

**Key Insight**: This system defines CSS patterns and tokens, not rigid components. The slot architecture provides vocabulary for content arrangement, but actual implementation is flexible composition of CSS classes and structural HTML. When existing patterns don't fit, the solution is usually:
- Compose existing classes differently
- Add a variant class for the specific need
- Create new CSS only when genuinely novel
...
<head>
  ...
  <style>
    /* @PRESERVE â€” Odyssey Lab Design Tokens v0.3 */
    :root {
...
## Appendix C: Quick Reference (Condensed)

> **Usage**: Copy this section for shorter context windows or rapid prompting.

```markdown
# Odyssey Design System â€” Quick Reference v0.3

## Essential Tokens
--color-bronze: #B48E55 (brand accent)
--color-lab-blue-electric: #2563EB (light zone interactive)
--color-lab-blue: #38BDF8 (dark zone interactive)
--light-bg-body: #F5F5F7 | --dark-bg-deep: #0F172A
...
## Core Classes
.zone-light / .zone-dark â€” Apply to container
.heading-hero / .heading-2 / .heading-3 â€” Titles
.eyebrow â€” Pre-header (mono, uppercase, bronze)
.body-lg / .body-base â€” Paragraphs
.card / .card--elevated / .card--accent-left â€” Surfaces
.btn--primary / .btn--secondary â€” Actions

## Hero Variants
.hero--full (100vh) | .hero--prominent (60vh)
.hero--compact (auto) | .hero--minimal (auto, left-aligned)

## Slot Order (visual flow)
leading â†’ badge/meta â†’ eyebrow â†’ heading â†’ subheading â†’ body â†’ supporting â†’ media â†’ actions â†’ action-cue â†’ footer
Note: eyebrow OR subheading, not both

## Rules
- Never hardcode hex/rgb â€” use var(--token)
- Keep @PRESERVE comments
- Flag new patterns with @CANDIDATE
...
<!-- @PRESERVE â€” End of Odyssey Lab Design System v0.3 -->
