...
import { ChevronDown, ChevronUp, Map, BookOpen, Share2, Compass, Layers, Globe, Zap, Heart, Anchor, Feather, Star } from 'lucide-react';

// --- STYLES & ANIMATIONS ---
const styles = `
  @import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@400;500;700&family=Inter:wght@300;400;500;600&family=JetBrains+Mono:wght@400;500&display=swap');
  ...
  /* Utility for gold text gradient */
  .text-gold-gradient {
  ...
  .border-gradient-gold {
    border-image: linear-gradient(to right, transparent, #B48E55, transparent) 1;
  }
`;
...
const SectionHeader = ({ kicker, title, description, dark = false }) => (
  <div className="mb-16 text-center">
    {kicker && (
      <span className={`font-mono text-xs tracking-[0.2em] uppercase mb-4 block ${dark ? 'text-blue-400' : 'text-blue-600'}`}>
        {kicker}
      </span>
    )}
    ...
    <div className={`h-px w-24 mx-auto mt-8 ${dark ? 'bg-[#B48E55]' : 'bg-[#D4AF37]'}`} />
  </div>
);

const AccordionItem = ({ title, subtitle, children, isOpen, onClick, dark = false }) => (
  <div className={`mb-4 border-l-2 transition-all duration-300 ${
    isOpen 
      ? (dark ? 'border-[#D4AF37] bg-[#1E293B]' : 'border-[#B48E55] bg-white shadow-lg') 
      : (dark ? 'border-gray-700 bg-[#0F172A] hover:border-gray-500' : 'border-gray-200 bg-gray-50 hover:bg-white')
  }`}>
    <button
      onClick={onClick}
    ...
    >
      <div>
        ...
        {subtitle && (
          <p className={`font-body text-sm mt-1 ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
            {subtitle}
          </p>
        )}
      </div>
    ...
    >
      <div className={`px-6 pb-6 pt-2 font-body leading-relaxed ${dark ? 'text-gray-300' : 'text-gray-600'}`}>
        {children}
      </div>
    </div>
  </div>
...
const PillarCard = ({ title, subtitle, icon: Icon, points }) => (
  <div className="bg-[#1E293B] border border-gray-800 p-8 h-full flex flex-col relative overflow-hidden group hover:border-[#B48E55] transition-colors duration-300">
    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
      <Icon size={80} />
    </div>
    <div className="font-mono text-xs text-[#B48E55] mb-4 uppercase tracking-widest">Foundation</div>
    <h3 className="font-display text-2xl text-white mb-2">{title}</h3>
    <p className="font-body text-gray-400 mb-6 italic">{subtitle}</p>
    
    <ul className="space-y-3 mt-auto">
      {points.map((point, i) => (
        <li key={i} className="flex items-start text-sm text-gray-300">
          <span className="text-[#B48E55] mr-2">•</span>
          {point}
        </li>
      ))}
    </ul>
...
const Quote = ({ text, author, citation }) => (
  <blockquote className="my-6 pl-4 border-l-4 border-[#B48E55] italic text-lg">
    "{text}"
    {(author || citation) && (
      <footer className="mt-2 text-sm not-italic opacity-70">
        {author && <span className="font-bold block">— {author}</span>}
        {citation && <span className="font-mono text-xs">{citation}</span>}
      </footer>
    )}
  </blockquote>
);
...
const Badge = ({ children }) => (
  <span className="inline-block px-3 py-1 mb-6 border border-[#B48E55] text-[#B48E55] text-xs font-mono tracking-widest uppercase rounded-sm">
    {children}
  </span>
);
...
const StickyNav = () => {
  ...
  return (
    <nav className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 bg-[#0F172A]/90 backdrop-blur-md border border-gray-800 rounded-full px-6 py-3 shadow-2xl flex gap-6 overflow-x-auto max-w-[90vw]">
      {navItems.map((item) => (
        <a 
          key={item.id} 
          href={`#${item.id}`} 
          className="text-xs font-mono uppercase tracking-wider text-gray-400 hover:text-[#D4AF37] transition-colors whitespace-nowrap"
        >
          {item.label}
        </a>
      ))}
...
export default function LifePhilosophy() {
  const [openPrinciples, setOpenPrinciples] = useState(0); // First open by default
  const [openManifestations, setOpenManifestations] = useState(null);
  ...
  const toggleManifestation = (index) => setOpenManifestations(openManifestations === index ? null : index);
    ...
    <div className="min-h-screen">
      ...
      <Section id="origin" className="bg-[#F5F5F7] text-gray-900">
        ...
        <div className="grid md:grid-cols-12 gap-12 items-start">
          ...
          <div className="md:col-span-8 font-body text-lg text-gray-700 space-y-6">
            ...
            <p>
              It is v0.9 because refinement remains. Questions persist. But the foundation is solid—coherent enough to build upon, authentic enough to stand behind.
            </p>
          </div>
        </div>
      </Section>
      ...
      <Section id="root" className="bg-[#0F172A]">
        ...
        <div className="max-w-3xl mx-auto border-t border-gray-800 pt-8">
          <h3 className="font-mono text-xs text-center text-gray-500 mb-8 uppercase tracking-widest">Deep Dive into Sources</h3>
          
          <AccordionItem 
            dark 
            title="1. Viktor Frankl: The Existential Foundation" 
            subtitle="Responsibility as the vehicle for meaning."
            isOpen={openSources === 0} 
            onClick={() => toggleSource(0)}
          >
            <p className="mb-4">Frankl provides the existential ground: Life has inherent meaning waiting to be discovered. Your task is response-ability. This grounds the entire philosophy in active engagement rather than passive consumption.</p>
            <Quote 
              text="When we are no longer able to change a situation, we are challenged to change ourselves."
              author="Viktor Frankl"
            />
            <p className="mt-4 text-sm text-gray-400"><strong>Integration:</strong> The concept that meaning is discovered through responsibility and response. That life actively questions us. That suffering can be purposeful rather than meaningless.</p>
          </AccordionItem>

          <AccordionItem 
            dark 
            title="2. Eckhart Tolle: The Mechanics of Consciousness" 
            subtitle="Presence as the path to clarity."
            isOpen={openSources === 1} 
            onClick={() => toggleSource(1)}
          >
            <p className="mb-4">Tolle provides the consciousness framework: How to actually BE in the world moment-to-moment. How to dissolve ego patterns that create suffering. How to access deeper awareness beneath thought.</p>
            <Quote 
              text="Realize deeply that the present moment is all you have. Make the NOW the primary focus of your life."
              author="Eckhart Tolle"
            />
            <p className="mt-4 text-sm text-gray-400"><strong>Integration:</strong> Present moment awareness as foundation. Ego as obstacle to clarity. Surrender as path to peace.</p>
          </AccordionItem>

          <AccordionItem 
            dark 
            title="3. Panpsychism: The Cosmological Ground" 
            subtitle="Reality is fundamentally interconnected."
            isOpen={openSources === 2} 
            onClick={() => toggleSource(2)}
          >
            <p className="mb-4">Panpsychism provides the cosmological foundation: What reality fundamentally IS. Why interconnection matters. How individual consciousness relates to the universal field. This is the metaphysical ground beneath the practical philosophy.</p>
            <div className="bg-[#1E293B] p-4 border-l-2 border-[#B48E55] my-4">
              <p className="text-sm italic">"Consciousness all the way down"—not just humans, but a fundamental property of the universe.</p>
            </div>
            <p className="mt-4 text-sm text-gray-400"><strong>Integration:</strong> Interconnection as reality (separateness as illusion). Consciousness as fundamental rather than emergent. The participatory nature of reality.</p>
          </AccordionItem>
        </div>
      </Section>
      ...
      <Section id="cosmology" className="bg-[#020617]">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="space-y-8 text-lg md:text-xl text-gray-300 font-light leading-relaxed text-left border-l border-gray-800 pl-8 md:pl-12">
            ...
            <p>
              You are not a separate observer of reality but an active participant shaping experience through consciousness. Your awareness affects what manifests.
            </p>
          </div>
        </div>
      </Section>
      ...
      <Section id="principles" className="bg-[#F5F5F7]">
        ...
        <div className="max-w-3xl mx-auto">
          {[
            {
              title: "1. Presence Over Productivity",
            ...
            {
              title: "2. Interconnection vs Separateness",
            ...
            {
              title: "3. Response-Ability Over Victimhood",
            ...
            {
              title: "4. Surrender Without Passivity",
            ...
            {
              title: "5. Ego Awareness & Dissolution",
            ...
            {
              title: "6. Meaning Through Service",
            ...
            {
              title: "7. Consciousness Elevation as Practice",
            ...
            {
              title: "8. Transmutation Over Transformation",
            ...
            {
              title: "9. Present-Future Duality",
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-4">
             ...
             {[
               ...
               <div key={i} className="bg-white p-6 border-l-4 border-[#D4AF37] shadow-sm hover:shadow-md transition-shadow">
                 <h4 className="font-display text-lg text-gray-900 mb-2">{item.title}</h4>
                 <p className="font-body text-gray-600 text-sm">{item.desc}</p>
               </div>
             ))}
          </div>
          <div className="space-y-4">
            {/* Right Column Manifestations */}
            {[
               ...
               <div key={i} className="bg-white p-6 border-l-4 border-[#B48E55] shadow-sm hover:shadow-md transition-shadow">
                 <h4 className="font-display text-lg text-gray-900 mb-2">{item.title}</h4>
                 <p className="font-body text-gray-600 text-sm">{item.desc}</p>
               </div>
             ))}
      ...
      <Section id="synthesis" className="bg-[#1E293B]">
         ...
         <div className="relative w-full max-w-lg mx-auto aspect-square flex items-center justify-center my-12">
            <svg viewBox="0 0 400 400" className="w-full h-full text-white opacity-90">
              ...
              {/* Circle 1: Frankl */}
              <circle cx="200" cy="130" r="90" fill="none" stroke="#D4AF37" strokeWidth="1.5" className="opacity-60" />
              ...
              {/* Circle 2: Tolle */}
              <circle cx="120" cy="270" r="90" fill="none" stroke="#D4AF37" strokeWidth="1.5" className="opacity-60" />
              ...
              <circle cx="280" cy="270" r="90" fill="none" stroke="#D4AF37" strokeWidth="1.5" className="opacity-60" />
         ...
         <div className="max-w-2xl mx-auto text-center text-gray-300 font-light space-y-4">
            <p>Frankl provides the <strong>foundation</strong> (Meaning).</p>
            <p>Tolle provides the <strong>mechanics</strong> (Presence).</p>
            <p>Panpsychism provides the <strong>ground</strong> (Connection).</p>
            <p className="pt-6 border-t border-gray-700 mt-6">
